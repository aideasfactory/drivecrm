...
// Where to send the user after a successful submission. The backend
// whitelists redirect_to to same-origin paths under /hmrc/itsa or
// /instructors/, so this can't be abused for an open redirect.
const successRedirectTarget = computed<string>(() =>
    props.instructorId
        ? `/instructors/${props.instructorId}?tab=hmrc&service=itsa`
        : '/hmrc/itsa',
);
...
const buildPayload = () => {
    const payload: Record<string, unknown> = {
        ...
        redirect_to: successRedirectTarget.value,
    };
    if (form.value.mode === 'consolidated') {
        ...
        <form class="flex flex-col gap-6" @submit.prevent="handleSubmit">
            ...
            <div class="flex justify-end gap-2">
                <Button as-child variant="outline" type="button">
                    <a :href="successRedirectTarget">Cancel</a>
                </Button>
