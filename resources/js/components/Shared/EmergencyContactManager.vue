...
const basePath = computed(() => `/${props.entityType}s/${props.entityId}/contacts`)
const entityLabel = computed(() => props.entityType === 'instructor' ? 'instructor' : 'student')
    ...
    try {
        if (isEditMode.value && editingContact.value) {
            const response = await axios.put(
                `${basePath.value}/${editingContact.value.id}`,
                form.value,
            )
            ...
            const response = await axios.post(basePath.value, form.value)
...
const handleDelete = async () => {
    ...
    try {
        await axios.delete(`${basePath.value}/${deleteContactId.value}`)
...
const handleSetPrimary = async (contact: Contact) => {
    ...
    try {
        await axios.patch(`${basePath.value}/${contact.id}/primary`)
                    ...
                    <SheetDescription>
                        {{
                            isEditMode
                                ...
                                : `Add a new emergency contact for this ${entityLabel}.`
                        }}
                    </SheetDescription>
                </SheetHeader>
