...
class RetrieveQuarterlyUpdateAction
{
     ...
     */
    public function __invoke(User $user, HmrcItsaQuarterlyUpdate $row, array $fraudContext = []): array
    {
        $instructor = $user->instructor;
        if ($instructor === null || ! is_string($instructor->nino)) {
            throw new HmrcApiException(
                message: 'Tax profile must include a NINO before retrieving an ITSA period.',
                statusCode: 400,
            );
        }
        ...
        if ($periodId === '') {
            throw new HmrcApiException(
                message: 'No HMRC submission id is on file for this period.',
                statusCode: 400,
            );
        }
