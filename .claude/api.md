...
| 2026-09-04 | **Lesson fees auto-post to the instructor Finances ledger when a lesson is paid.** New payment category `lesson_fee` ("Lesson Fee") on `GET /instructor/finances/config`. Weekly `invoice.paid` and upfront checkout create a `type=payment` finance row for the **lesson-fee portion only** (booking fee and digital charge excluded). Amount is `LessonPayment::weeklyBreakdown()['lesson']` for weekly invoices, or `lesson.amount_pence` for upfront. One row per `lesson_payment` (unique `instructor_finances.lesson_payment_id`). Appears on the instructor profile Finances tab and in `GET /instructor/finances`. This is a ledger entry, not a Stripe Connect transfer. | Finances (Config, List) |
